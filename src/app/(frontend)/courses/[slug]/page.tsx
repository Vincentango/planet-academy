...
export default async function CourseDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <article className="container-wide pb-16 pt-12">
      ...
      <h1 className="mt-3 text-4xl font-semibold">{course.title}</h1>
      ...
      <section className="mt-8 grid gap-4 md:grid-cols-3">
        {[
          ...
          <div key={String(label)} className="rounded-2xl border border-line bg-elevated p-4">
      ...
      <section className="mt-10">
        <h2 className="text-2xl font-semibold">模块时间线</h2>
        <ol className="mt-5 grid gap-4">
          {modules.map((mod, i) => (
            <li key={mod.id || i} className="rounded-2xl border border-line bg-card p-5">
              <p className="text-xs text-lime">模块 {i + 1}{mod.hours ? ` · ${mod.hours} 课时` : ''}</p>
