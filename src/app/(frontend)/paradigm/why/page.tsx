...
export default function WhyPage() {
  return (
    <>
      ...
      />
      <ParadigmNav />
      <section className="container-wide grid gap-6 pb-16">
        ...
        <h2 className="text-2xl font-semibold">七大人类独特能力</h2>
        <p className="max-w-3xl text-muted">这些能力是 AI 在可预见的未来无法真正复制的，构成人类不可替代性的核心。它们指向身体性、责任性、关系性和创造性。</p>
        <div className="grid gap-4 md:grid-cols-2">
          {HUMAN_UNIQUES.map((item) => (
            <article key={item.name} className="rounded-2xl border border-line bg-card p-5">
              <p className="text-xs text-lime">{item.n} {item.en}</p>
              <h3 className="mt-1 text-lg font-semibold">{item.name}</h3>
              <p className="mt-2 text-sm leading-7">{item.meaning}</p>
              <p className="mt-2 text-sm text-muted">{item.limit}</p>
            </article>
          ))}
        </div>
        <h2 className="text-2xl font-semibold">AI 重塑教育的三条路径</h2>
        <div className="grid gap-4 md:grid-cols-3">
          {[
        ...
        <h2 className="text-2xl font-semibold">四大人机协作边界</h2>
        <div className="grid gap-4 md:grid-cols-2">
          {AI_BOUNDARIES.map((item) => (
            <article key={item.title} className="rounded-2xl border border-lime/20 bg-forest p-5">
